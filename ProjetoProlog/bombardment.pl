...
imprimeTabuleiroJogador(Tabuleiro, TipoTabuleiro) :-
  write('############ BOMBARDMENT OF THE VIRUS ###########'),nl,nl,
  write('    A   B   C   D   E   F   G   H   I   J   L   M'),nl,nl,
  TipoTabuleiro =:= 0 -> imprimeMatrizJogadorGabarito(Tabuleiro, 0);
  TipoTabuleiro =:= 1 -> imprimeMatrizJogadorReal(Tabuleiro, 0);
  write('~ = Terreno | n = Area | X = TIRO PERDIDO').

imprimeMatrizJogadorGabarito([], _).
imprimeMatrizJogadorGabarito([H|T], Index) :-
    write(Index), write('  '), imprimeLinhaJogadorGabarito(H), nl,nl,
    NewIndex is Index+1,
    imprimeMatrizJogadorGabarito(T, NewIndex).

imprimeLinhaJogadorGabarito([]).
imprimeLinhaJogadorGabarito([H|T]) :-
  (H == '~', write('|_|');
  H == n, write('|o|');
  H == b, write('|*|');
  H == a, write('|#|');
  H == @, write('|X|');
  H == &, write('')), write(' '),
  imprimeLinhaJogadorGabarito(T).

imprimeMatrizJogadorReal([], _).
imprimeMatrizJogadorReal([H|T], Index) :-
    write(Index),
    write('  '), 
    imprimeLinhaJogadorReal(H), nl,nl,
    NewIndex is Index+1,
    imprimeMatrizJogadorReal(T, NewIndex).

imprimeLinhaJogadorReal([]).
imprimeLinhaJogadorReal([H|T]) :-
  (H == '~', write('|_|');
  H == n, write('~');
  H == x, write('x')), write(' '),
  imprimeLinhaJogadorReal(T).
...
jogar(Tabuleiro, Misseis) :-
  Misseis > 0,
  imprimeTabuleiroJogador(Tabuleiro, 1),
